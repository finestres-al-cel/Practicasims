...
C---------------------------------------    
     ...
     *     amg(Nmaxpart),eps(Nmaxpart),epss(Nmaxpart) 
      ...
      rscale = 2.87e4 !distancia en kpc
      vscale = 691.0 !velo en km/s
      tscale = rscale/vscale*0.979 !temps en Gyrs
      amscale = 3.17e15 ! massa en msun
...
c       eps_def = 0.09/rscale ! khtk
 ...
 13    if(i.le.lspecies(k))then  
       ...
       xc(i) = xc(i)*Rs/hubble/rscale*2.0e3 
       yc(i) = yc(i)*Rs/hubble/rscale*2.0e3       
       zc(i) = zc(i)*Rs/hubble/rscale*2.0e3 
          
       vxc(i) = vxc(i)*208.e-5*sqrt(aMassh/Fcon/Rs)/vscale*2.0e2 
       vyc(i) = vyc(i)*208.e-5*sqrt(aMassh/Fcon/Rs)/vscale*2.0e2  
       vzc(i) = vzc(i)*208.e-5*sqrt(aMassh/Fcon/Rs)/vscale*2.0e2 
       amg(i)=amg(i)*8.0e7   
      ...
      do i=1,nt
      write(10,1111)xc(i),yc(i),zc(i),vxc(i),vyc(i),vzc(i),0.,
     +0.,0.,amg(i) 
      enddo 
      close(10)
 1111 format(10(f24.15,1x))
      return    
      end  
