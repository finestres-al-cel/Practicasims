...
c               write snapshot of the system to file
      SUBROUTINE WriteSnapshot(yc,time)
c-------------------------------------------------------------------- 
      INCLUDE 'nbody1.h'
      REAL*8 yc(3)
      ...
      write(FileNAMES(1),'(a,i3.3,".",i3.3,a)')path//'Snap',i1,i2,'.dat'
...
c         write(1)
c     &      Npi,dt,time,iStep,epsilon,RunTime,
c     &                 PrintTime
c         write(30,10)
c     &      Npi,dt,time,iStep,epsilon,RunTime,
c     &                 PrintTime
...
c-------------------------------------------------------------------- 
c               
      SUBROUTINE Analyze(indx,yc,time)
c-------------------------------------------------------------------- 
      PARAMETER (Nbin =30)  ! number of bins for mass profile
      INCLUDE 'nbody1.h'
     ...
     &           5.0,10.0,15.0,20.0,30.0,40.0,50.0,75.0,100.0,
     &           125.0,150.0,175.0,200.0,400.0,750.0,1000.0/

         i1   = INT(time)
         i2   = MIN(INT((time-i1)*1000.+0.5),999)
      write(FileNAMES(1),'(a,i3.3,".",i3.3,a)')'DATA/Prof',i1,i2,'.dat'
c      open(9,file=FileNAMES(1))
 ...
 120   format(T48,32f8.2)
      ...
      Do iter=1,150
         ...
         If(Dens1.lt.Dens0.or.inside.lt.Npi/3)goto300
         Dens0 = Dens1
      EndDo 
 ...
 300  Do i=1,Nbin
      ...
      Do i=1,Npi      ! get statistics relative to the center      
         aM      = Coords(10,i)
         dx      = Coords(1,i) -yc(1)
         dy      = Coords(2,i) -yc(2)
         dz      = Coords(3,i) -yc(3)
 ...
 101   format(f8.4,i8,2f8.3,2f8.3,32f8.4)
                            
 110   format(i4,2g12.4,3x,2g12.4,3x,g12.3,i5,4x,4f8.3)
                   !  write density profile
cc       write(9,100)' t=',time,' Step=',iStep,
cc     &                    ' Mass=',aMass,aMass2,
cc     &                   ' Rmax/Raverage=',Rmax,Raverage
cc       write(9,120)
       aMass2 =0.
       Do i=-100,Nbin
          If(nDen(i).gt.0)Then
             ...
             den= aM/vol
             dn = nDen(i)/vol
c             write(9,110)i,rr,rin,den,dn,aM/nDen(i),nDen(i),
c     &                   vr,v2,aMass2
          EndIf 
       EndDo 
c       close(9)
